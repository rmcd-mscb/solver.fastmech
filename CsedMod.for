...
c	 write(6,*) 'Yalin (0) or Engelund-Hansen (1):'
c	 read(5,*) ityp
...
c	              CON(I,J)=(QS(2,J)-QS(1,J))/(ds*SC)
			         CON(I,J)=CON(I-1,J)
c			         CON(I,J)=CON(I,J)-(QN(I,J)/(R(I)*SC))
			         GO TO 643
			    ELSE
...
c             DUM1(I,1)=(.5*CON(I-1,1)+CON(I,1)+.5*CON(I+1,1))/2.
       		elseif(j.eq.nn) then
           DUM1(I,nn)=(CON(I-1,nn)+CON(I,nn)+CON(I+1,nn)+con(i,nn-1))/4.
c             DUM1(I,nn)=(.5*CON(I-1,nn)+CON(I,nn)+.5*CON(I+1,nn))/2.
     			else
			weight=1
