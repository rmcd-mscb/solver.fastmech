...
SUBROUTINE CGNS2_Read_CalcCondtion_ForAlloc(IER)
IMPLICIT NONE
...
CALL cg_iRIC_Read_Integer_f('FM_SolAttType', solType, ier)

CALL cg_iRIC_Read_Real_F('FM_SolAttERlx', erelax, ier)
CALL cg_iRIC_Read_Real_F('FM_SolAttURlx', urelax, ier)
CALL cg_iRIC_Read_Real_F('FM_SolAttARlx', arelax, ier)
...
CALL CG_IRIC_READ_INTEGER_F('FM_SolAttPlinc', iplinc, ier)

!NEXT TWO DEFINED IN RivVarTimeMod
CALL cg_iRIC_Read_Real_F('FM_SolAttVarDischSTime', vardischstarttime, ier)
CALL cg_iRIC_Read_Real_F('FM_SolAttVarDischETime', vardischendtime, ier)
...
CALL CG_IRIC_READ_INTEGER_F('FM_SolAttDbgStop', debugstop, ier)
CALL CG_IRIC_READ_INTEGER_F('FM_SolAttDbgTimeStep', DbgTimeStep, ier)
CALL CG_IRIC_READ_INTEGER_F('FM_SolAttDbgIterNum', DbgIterNum, ier)
...
CALL CG_IRIC_READ_INTEGER_F('FM_SolAttIterOut', tmpint, ier)
    ...
    ELSE
        IterationOut = .FALSE.
    ENDIF
CALL CG_IRIC_READ_INTEGER_F('FM_SolAttIncPlOut', IterPlOut, ier)
...
CALL cg_iRIC_Read_Real_F('FM_HydAttWS', wselev, ier)
    wselev=(wselev - elevoffset)*100.
CALL cg_iRIC_Read_Real_F('FM_HydAttWS2', wsupelev, ier)
    wsupelev=(wsupelev - elevoffset)*100.
CALL cg_iRIC_Read_Real_F('FM_HydAttWSSlope', wsslope, ier)
CALL CG_IRIC_READ_INTEGER_F('FM_HydAttWSType', wstype, ier)
    IF(wstype == 3) THEN
        HotStart = .TRUE.
    ELSE
        HotStart = .FALSE.
...
CALL cg_iRIC_Read_Real_F('FM_HydAttCD', constcd, ier)
CALL cg_iRIC_Read_Real_F('FM_HydAttCDMin', cdmin, ier)
...
CALL cg_iRIC_Read_Real_F('FM_EndLEV', endLEV, ier)
    endLEV = endLEV*100*100
CALL cg_iRIC_Read_Real_F('FM_HydAttEVC', evc, ier)
    evc = evc*100.*100.
...
CALL cg_iRIC_Read_Real_F('FM_Quasi3D_MinRS', MinRS, ier)
	MinRs = MinRs * 100.
...
CALL CG_IRIC_READ_INTEGER_F('FM_SedTrans', tmpint, ier)
    ...
    ELSE
        CALCCSED = .FALSE.
    ENDIF
CALL cg_iRIC_Read_Real_F('FM_SedDuneH', HD, ier)
    HD = HD * 100.
...
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_RG0', taustar_rg0, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_RG1', taustar_rg1, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_RS1', taustar_rs1, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_Alpha', alpha, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_AK', AK, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_Chi', Chi, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_PhiPrime', phi_prime, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_HMax', tmphmax, ier)
	tmphmax = tmphmax * 100.
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_ABH', tmpabh, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_BBH', tmpbbh, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_LSub', tmplsub, ier)
	tmplsub = tmplsub*100.
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_LSubActDepth', tmplsubactdepth, ier)
	tmplsubactdepth = tmplsubactdepth*100.
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_DSand', Dsand, ier)
	Dsand = Dsand *100.
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_DGravel', Dg, ier)
	Dg = Dg*100.
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_Z0Min', Z0Min, ier)
	Z0Min = Z0Min*100.
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_Z0Max', Z0Max, ier)
	Z0Max = Z0Max*100.		

CALL CG_IRIC_READ_INTEGER_F('FM_Sed_WK_RoughType', WK_RoughType, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_RoughShapeParam', WK_RoughShapeParam, ier)
CALL cg_iRIC_Read_Real_F('FM_Sed_WK_FsMin', Fsmin, ier)
