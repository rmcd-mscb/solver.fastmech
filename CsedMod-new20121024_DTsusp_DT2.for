...
!612			CONTINUE
		...
		if (TRANSEQTYPE.eq.1) THEN
		    ...
		    go to 777         !Daniele 
		else if(TRANSEQTYPE.eq.3) THEN
		    go to 651 ! Meyer-Peter Muller
        ENDIF
!		!!!!  YALIN EQUATION   !!!!!
...
651     do  i=1,ns
			do  j=1,nn	
			    TB=((USTRESS(I,J)**2)+(VSTRESS(I,J)**2))**.5
			    TC = 0.047*1.65*980.0*D(I,J)
			    QS(I,J) = 8*((TB/(1.65*980.*D(I,J)))-0.047)**1.5
			    QS(I,J) = QS(I,J) * D(I,J) * SQRT(1.65*980.0*D(I,J))
			    QS(I,J)=QS(I,J)*USTRESS(I,J)/(TB)
			    TAN1=VSTRESS(I,J)/USTRESS(I,J)
			    if(CalcGravCorr .AND. GRAVCORRTYPE == 1) then
				    TAN2=GRAVFLATBEDCORRCOEF*((TC/TB)**.5)*DHDN(I,J)
...
!			                dum1(i,j) = 0.
!			                
!			            ELSEIF(J.eq.1.or.j.eq.nn) THEN
!			                dum1(i,j) = 0
!			            ELSE
!			                IF(ibc(i,j).eq.-1) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i,j)
!			                ENDIF
!			                IF(ibc(i-1,j).eq.-1.and.i.gt.1) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i-1,j)
!			                ENDIF
!			                IF(ibc(i-1,j-1).eq.-1.and.i.gt.1) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i-1,j-1)
!			                ENDIF
!			                IF(ibc(i-1,j+1).eq.-1.and.i.gt.1) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i-1,j+1)
!			                ENDIF
!			                IF(ibc(i+1,j).eq.-1.and.i.lt.ns) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i+1,j)
!			                ENDIF
!			                IF(ibc(i+1,j-1).eq.-1.and.i.lt.ns) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i+1,j-1)
!			                ENDIF
!			                IF(ibc(i+1,j+1).eq.-1.and.i.lt.ns) THEN
...
!			                    MedianArray(countmd) = con(i+1,j+1)
!			                ENDIF
!			                IF(ibc(i,j-1).eq.-1) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i,j-1)
!			                ENDIF
!			                IF(ibc(i,j+1).eq.-1) THEN
!			                    countmd = countmd+1
!			                    MedianArray(countmd) = con(i,j+1)
!			                ENDIF
!			                IF(countmd == 1) THEN
!			                    dum1(i,j) = MedianArray(1)
!			                ELSEIF(countmd == 2) THEN
!			                    dum1(i,j) = (MedianArray(1)+MedianArray(2))/2.
!			                ELSE
!			                    CALL MDIAN(MedianArray,countmd,XMED)
!			                    dum1(I,j) = XMED
!			                ENDIF
!			            ENDIF
!			            
...
!	
     ...
     &                  , Fsmin, Fracs(I,J), harea(i,j), hfine(I,J), he)
			...
			if(he.GT.hl(i,j).and.ibc(i,j).ne.0) then
...
!     account for unerodible elevation
			if(eta(i,j) < mineta(i,j))then
			    if(con(i,j).ne.0) then
			    eta(i,j) = mineta(i,j)
			    endif
			endif
