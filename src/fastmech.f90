    ...
    if(vardischendtime == 0 .and. vardischstarttime == 0)then
        nsteps = 0
    else
    ...
    nct = -1
    model%t => vardischstarttime
    model%dt => fmdt
    model%t_end => vardischendtime
    ptime = model%t+iplinc*model%dt
    ...
    model%n_y => nn
    model%n_z => nz
    model%fm_elevation => eta
    model%fm_depth => hl
    model%fm_wse => e
    model%fm_velocity_x => u
    model%fm_velocity_y => v
    model%fm_shearstress_x => taus
    model%fm_shearstress_y => taun
    model%fm_dragcoefficient => totcd
    model%fm_roughness => cd
    model%fm_vegroughness => cdv
    !model%t = 0.
    !model%dt = 1.
    !model%dx = 1.
    !model%dy = 1.
    ...
    if(vardischtype == 1) then !update discharge
        call getinterptimeseriesvalue(1, model%t, newdisch)
        q = newdisch*1e6
    endif
    ddisch = q-olddisch

    if(varstagetype == 1) then !stage time series
        call getinterpratingcurvevalue(1, model%t, newstage)
        ...
        if(i_re_flag_o.eq.1.and.nct.eq.0) then
            if(i_tmp_count <= n_rest) then
                if(iter.eq.opt_tmp(i_tmp_count)) then
                    ...
                    !
                    write(502) model%t,solindex,fmdt
                    write(502) ns,nn
                    !
