    ...
    CONTAINS
    SUBROUTINE CGNS2_READ_GRIDCOORD(model, IER)
    IMPLICIT NONE
    !	INCLUDE "cgnslib_f.h"
    !    INCLUDE "cgnswin_f.h"
    ...
    model%rivvar%ns2 = tmpns
    model%rivvar%ns = tmpns+nsext
    model%rivvar%nn = tmpnn
    ...
    SUBROUTINE CalcGridMetrics(model)
    IMPLICIT NONE
    ...
    INTEGER :: jmid
    tnn = model%t_rivvar%nn
    tns = model%t_rivvar%ns
    jmid = (model%t_rivvar%nn+1)/2
    r2 = 0
    ! Find length along centerline
    model%t_rivvar%stot = 0.
    DO i = 2,model%t_rivvar%ns2
        ds = sqrt((model%t_rivvar%x(i,jmid)-model%t_rivvar%x(i-1,jmid))**2 + (model%t_rivvar%y(i,jmid)-model%t_rivvar%y(i-1,jmid))**2)
        model%t_rivvar%stot = model%t_rivvar%stot+ds
    ENDDO


    model%t_rivvar%scals = model%t_rivvar%stot/(model%t_rivvar%ns2-1)
    model%t_rivvar%xshift = model%t_rivvar%x(1,jmid)
    model%t_rivvar%yshift = model%t_rivvar%y(1,jmid)

    tmpwidth = sqrt((model%t_rivvar%x(1,1)-model%t_rivvar%x(1,tnn))**2 + (model%t_rivvar%y(1,1)-model%t_rivvar%y(1,tnn))**2)
    DO i = 1,model%t_rivvar%ns2
        xo(i) = model%t_rivvar%x(i,jmid)-xshift
