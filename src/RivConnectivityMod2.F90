    MODULE RivConnectivityMod2
    USE RivVarMod2
    IMPLICIT NONE

    CONTAINS
    SUBROUTINE Label_clusters(rvo, selected_state, state, nclusters, cluster_labels)
    IMPLICIT NONE
    ...
    DO i = 1,ns
        DO j = 1,nn
            IF(Node_needs_label(selected_state, state(i,j), cluster_labels(i,j))) THEN
                ...
                CALL Label_neighborhood(rvo,i,j,state(i,j), state, nclusters, cluster_labels)
            ENDIF
        ENDDO
    ENDDO
    ...
    RECURSIVE SUBROUTINE Label_neighborhood(rvo, i, j, selected_state, state, label, cluster_labels)
    IMPLICIT NONE
    type(rivvar), intent(in) :: rvo
    ...
    IF(i.ne.1) THEN
        IF(Node_needs_label(selected_state, state(i-1, j), cluster_labels(i-1,j))) THEN
            CALL Label_neighborhood(rvo, i-1,j, selected_state, state, label, cluster_labels)
        ENDIF
    ENDIF
    if(i.ne.ns) then
        IF(Node_needs_label(selected_state, state(i+1, j), cluster_labels(i+1,j))) THEN
    ...
    if(j.ne.1)then
        IF(Node_needs_label(selected_state, state(i, j-1), cluster_labels(i,j-1))) THEN
            CALL Label_neighborhood(rvo, i,j-1, selected_state, state, label, cluster_labels)
        ENDIF
    endif
    ...
    SUBROUTINE Delete_clusters(rvo, ibc, cluster_labels, nclusters, nwetnodes)
    IMPLICIT NONE
